...
require "json"
require "rack"
require "lib/contexts/web_context"
...
module Webapp
  module Helpers
    ...
    def params_hash(namespace)
      coerce_hash(req.params[namespace])
    end
    ...
    def render_file(path, status_code = 200, cached = false)
      ...
      if cached
        status_code = response[0] == 304 ? 304 : response[0]
      else
        response[1].delete("Last-Modified")
      end
      halt([status_code, response[1], response[2]])
    end
